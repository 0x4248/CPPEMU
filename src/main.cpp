...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < instructions.size(); i++) {
        ...
        if (instructions[i] == "00") {
            ...
            call_debugger("ADD A " + to_string(reg_a) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "4B") {
            memory[location] = memory[location] + reg_b;
            call_debugger("ADD B " + to_string(reg_b) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "4C") {
            memory[location] = memory[location] + reg_c;
            call_debugger("ADD C " + to_string(reg_c) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "4D") {
            memory[location] = memory[location] + reg_d;
            call_debugger("ADD D " + to_string(reg_d) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "5A") {
            memory[location] = memory[location] - reg_a;
            call_debugger("MIN A " + to_string(reg_a) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "5B") {
            memory[location] = memory[location] - reg_b;
            call_debugger("MIN B " + to_string(reg_b) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "5C") {
            memory[location] = memory[location] - reg_c;
            call_debugger("MIN C " + to_string(reg_c) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "5D") {
            memory[location] = memory[location] - reg_d;
            call_debugger("MIN D " + to_string(reg_d) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "6A") {
            memory[location] = memory[location] * reg_a;
            call_debugger("MUL A " + to_string(reg_a) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "6B") {
            memory[location] = memory[location] * reg_b;
            call_debugger("MUL B " + to_string(reg_b) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "6C") {
            memory[location] = memory[location] * reg_c;
            call_debugger("MUL C " + to_string(reg_c) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "6D") {
            memory[location] = memory[location] * reg_d;
            call_debugger("MUL D " + to_string(reg_d) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "7A") {
            memory[location] = memory[location] / reg_a;
            call_debugger("DIV A " + to_string(reg_a) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "7B") {
            memory[location] = memory[location] / reg_b;
            call_debugger("DIV B " + to_string(reg_b) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "7C") {
            memory[location] = memory[location] / reg_c;
            call_debugger("DIV C " + to_string(reg_c) + " -> " +
                          to_string(location));
        } else if (instructions[i] == "7D") {
            memory[location] = memory[location] / reg_d;
            call_debugger("DIV D " + to_string(reg_d) + " -> " +
                          to_string(location));
        }
    ...
    nice_verbose_text::ok(
        "Ran out of instructions to execute. CPU Stopped end of program");
    return 0;
}
